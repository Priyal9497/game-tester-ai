"""
TestProbe AI - Selenium Game Testing Engine (FIXED)
"""
...
KNOWN_GAME_DOMAINS = [
    ...
    "abcya.com", "turtlediary.com", "primarygames.com"
]

# ── Known Non-Game Sites ──────────────────────────────────────
KNOWN_NON_GAME_DOMAINS = [
    ...
    "twitch.tv", "zoom.us", "slack.com",
    "notion.so", "dropbox.com"
]
...
def find_chromedriver_manually():
    """
    Search WDM cache for chromedriver
    """
    ...
    if not found:
        logger.warning("No chromedriver in WDM cache")
        return None
...
def fix_wdm_path(raw_path):
    """
    Fix WDM returning wrong file path
    """
    if not raw_path:
        return raw_path
    ...
    logger.info(f"Fixing WDM path in: {folder}")
    ...
    logger.error("Could not fix WDM path")
    return raw_path
...
def create_driver():
    ...
    # ✅ Required for Render/server environment
    ...
    # ✅ FIXED — Check multiple env variable names
    # Support both Docker (CHROME_BIN) and Render env vars
    chrome_binary = (
        os.getenv("CHROME_BIN") or          # Docker Dockerfile
        os.getenv("GOOGLE_CHROME_BIN") or   # Render buildpack
        os.getenv("CHROME_BINARY")          # Custom
    )

    chromedriver_path = (
        os.getenv("CHROMEDRIVER_PATH") or   # Docker / Render
        os.getenv("CHROMEDRIVER_BIN")       # Alternative
    )
    ...
    if chrome_binary and os.path.exists(chrome_binary):
        ...
        logger.info("No Chrome binary env var — using system default")

    # ── ChromeDriver Priority ──────────────────────────────────
    ...
    if chromedriver_path and os.path.exists(chromedriver_path):
        ...
        if driver_exe:
            logger.info(f"Using manual ChromeDriver: {driver_exe}")
            service = Service(executable_path=driver_exe)

        else:
            # Priority 3: webdriver-manager auto download
            try:
    ...
    try:
        ...
        # ✅ Non-fatal — continue without fingerprint removal
        logger.warning(f"CDP fingerprint removal failed: {e}")
...
def is_game_page_authentic(driver, url):
    """
    Deep check if page is actually a game.
    True  = confirmed game
    ...
    try:
        ...
        # Step 6: Canvas detected
        if page_info.get("has_canvas"):
            return True, (
                f"Canvas detected "
                f"({page_info.get('canvas_count', 0)} elements)"
            )
        ...
        for indicator in game_url_indicators:
            if indicator in url_lower:
                return True, f"Game path in URL"
        ...
        try:
            ...
            for sel in game_selectors:
                if driver.find_elements(By.CSS_SELECTOR, sel):
                    return True, f"Game element found"
        except Exception:
            pass

        # Step 10: Body text keywords
        try:
...
def extract_score(driver):
    """
    ✅ FIXED — No fake scores, returns None if not found
    """
    try:
        from selenium.webdriver.common.by import By

        # JavaScript check first
        js_vars = [
            "window.score",
            "window.Score",
            "window.points",
        ...
        # ✅ FIXED — Use try/finally to always restore implicitly_wait
        try:
            ...
            # ✅ Always restore — even if exception occurs
            driver.implicitly_wait(2)

    except Exception:
        pass

    # ✅ FIXED — Return None, never fake a score
    return None
...
def calculate_performance(
    ...
    # ✅ FIXED — NotGame for error/load failure
    if load_failed or is_error_pg:
        return "NotGame"

    # ✅ FIXED — NotGame if no actions AND very short time
    # (means page didn't actually load as a game)
    if actions == 0 and time_survived < 5:
        return "NotGame"

    # Actions = 0 but page loaded = Low
    if actions == 0:
        return "Low"
    ...
    action_rate = actions / max(float(time_survived), 1.0)
    if action_rate >= 1.0:
        score += 30
    ...
    # ✅ FIXED — Only use real scores for growth calc
    real_scores = [s for s in scores if s > 0]
    if len(real_scores) > 1:
        growth = real_scores[-1] - real_scores[0]
        if growth > 30:
            score += 20
        elif growth > 10:
            score += 12
...
def detect_game_type(driver, url, page_info, metrics):
    ...
    game_types = {
        "Endless Runner": {
            "keywords": [
                "runner", "endless", "dino", "run",
                "jump", "avoid", "obstacle"
            ],
            "url_indicators": ["runner", "endless", "dino", "run"],
            "behavior":       "high_action_rate",
            "description":    "Player continuously runs avoiding obstacles."
        },
        "Puzzle": {
            "keywords": [
                "puzzle", "match", "brain", "quiz",
                "sudoku", "2048", "candy"
            ],
        ...
        "Action/Shooter": {
            "keywords": [
                "shoot", "gun", "battle", "fight",
                "combat", "zombie", "war"
            ],
            "url_indicators": ["shooter", "action", "fps", "battle"],
            "behavior":       "high_action_rate",
            "description":    "Fast-paced shooting or combat mechanics."
        },
        "Racing": {
            "keywords": [
                "race", "racing", "car", "drive",
                "speed", "motor", "bike"
            ],
        ...
        "Card/Board": {
            ...
            "url_indicators": ["card", "poker", "chess", "board"],
            "behavior":       "low_action_rate",
        ...
        "Arcade/Classic": {
            ...
            "url_indicators": ["arcade", "classic", "retro", "pacman"],
            "behavior":       "high_action_rate",
    ...
    action_rate = (
        ...
        max(float(metrics.get("time_survived", 1)), 1.0)
    )
    ...
    for game_type, data in game_types.items():
        ...
        if data["behavior"] == "high_action_rate" and action_rate > 0.8:
            ...
            data["behavior"] == "moderate_actions"
            and 0.3 < action_rate <= 0.8
        ):
            type_score += 10
...
def run_test(url):
    ...
    # ✅ FIXED — Start with empty list, not [0]
    scores         = []
    page_info      = {}
    load_failed    = False
    ...
    try:
        ...
        if not is_browser_alive(driver):
            ...
            if not is_browser_alive(driver):
                ...
                if page_info.get("is_error_page"):
                    load_failed = True
                    logger.warning(
                        f"Error page: {page_info.get('title', '')}"
                    )
                ...
                if not load_failed:
                    ...
                    logger.info(
                        f"Waiting {GAME_LOAD_WAIT}s for game assets..."
                    )
                    ...
                    if not is_browser_alive(driver):
                        ...
                        # Action rounds
                        logger.info(
                            f"Running {ACTION_ROUNDS} action rounds..."
                        )
                        ...
                        for i in range(ACTION_ROUNDS):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Key send failed at action {i}"
                                    )

                                # ✅ FIXED — Only append REAL scores
                                real = extract_score(driver)
                                if real is not None:
                                    scores.append(real)
                                # No fake score appended if None
                                ...
                                logger.warning(
                                    f"Action {i} error: {str(e)[:50]}"
                                )
                                errors += 1
                                if errors >= MAX_ACTION_ERRORS:
                                    logger.warning(
                                        "Max errors - stopping actions"
                                    )
                                    break
        ...
        if driver:
            try:
                # ✅ FIXED — Always try game type detection
                # regardless of load_failed state
                if page_info and actions > 0:
                    try:
    ...
    # ✅ FIXED — Use empty list if no scores captured
    final_scores = scores if scores else []
