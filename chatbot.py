...
def is_likely_game_url(url: str) -> tuple:
    ...
    game_domains = [
        ...
        "agar.io", "diep.io", "papergames.io", "boardgamearena"
    ]
    
    # ✅ Game indicators in URL path
    game_indicators = [
        "/game/", "/play/", "/games/", "/arcade/", 
        "game.html", "play.html", "index.html", "/gameplay",
        "/online-game", "/browser-game", "/free-game"
    ]
    ...
    non_game_indicators = [
        ...
        "login", "signup", "register", "mail.", "docs.",
        "/wp-content", "/category/", "/tag/", "?p="
    ]
    
    # Quick check for obvious non-games
    for bad in non_game_domains:
        if bad in url_lower and (".com" in url_lower or ".net" in url_lower):
...
def build_reply(url: str, metrics: dict, ai_analysis: str) -> str:
    ...
    # Special handling for non-game links
    if perf == "NotGame":
        reason = metrics.get("game_check_reason", "No game detected")
        return (
            f"❌ WRONG LINK - CAN'T TEST THIS LINK\n\n"
            f"URL: {url}\n"
            f"{'─' * 40}\n"
            f"Reason: {reason}\n\n"
    ...
    game_type_confidence = game_type.get("confidence", "Low")
    game_type_desc = game_type.get("description", "")
    
    # Confidence emoji
    conf_emoji = {"High": "✅", "Medium": "🟡", "Low": "❓"}.get(game_type_confidence, "❓")
    
    return (
        f"✅ TEST COMPLETE!\n"
        f"URL: {url}\n\n"
        f"{'─' * 40}\n"
        f"🎮 GAME TYPE: {game_type_name} {conf_emoji}\n"
        f"   {game_type_desc}\n"
        f"{'─' * 40}\n"
        f"⏱ Time Survived  : {time_s:.1f} seconds\n"
        f"🎮 Actions Done   : {actions}\n"
...
def add_to_conversation(session_id: str, role: str, content: str):
    ...
    if len(conversation_memory[session_id]) > MAX_CONVERSATION:
        conversation_memory[session_id] = conversation_memory[session_id][-MAX_CONVERSATION:]
...
def run_game_test(url: str, session_id: str = "default") -> dict:
    ...
    # Pre-check URL before running Selenium
...
def process_message(message: str, session_id: str = None) -> dict:
    ...
    if url:
        # URL detected - run game test
        return run_game_test(url, session_id)
    
    # No URL found - treat as chat message
